/**
 * @file doctor-sidebar.tsx
 * @description Боковое меню врача в стиле Admin: бургер на мобильных, sidebar на десктопе
 * @created 2026-02-22
 */
...
import { AdminUsersIcon, AdminLinkIcon } from '@/components/admin/admin-icons';

const NAV_ITEMS = [
  { href: '/doctor/dashboard', label: 'Пациенты', Icon: AdminUsersIcon },
  { href: '/doctor/connect', label: 'Подключиться', Icon: AdminLinkIcon },
  { href: '/chat', label: 'Чат', Icon: BotIcon },
  { href: '/doctor/settings', label: 'Настройки', Icon: SettingsIcon },
];
          ...
          {NAV_ITEMS.map((item) => {
            const isActive =
              item.href === '/doctor/dashboard'
                ? pathname === '/doctor/dashboard' || pathname.startsWith('/doctor/patients')

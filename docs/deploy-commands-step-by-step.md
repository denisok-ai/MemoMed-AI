...
Страница `/dev-login` позволяет войти тестовыми аккаунтами (пароль `Test1234!`). На сервере включите её через переменные окружения:
...
NEXTAUTH_URL=http://91.201.41.28:3000
```

**Важно:** `NEXTAUTH_URL` должен совпадать с адресом, по которому вы открываете приложение (подставьте свой IP или домен). Иначе сессия не сохранится и после входа будет редирект на `/login`.
